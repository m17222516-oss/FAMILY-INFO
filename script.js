...
function close(){ sideMenu.classList.remove('open'); overlay.classList.remove('show'); }
menuBtn?.addEventListener('click', openMenu);
closeMenu?.addEventListener('click', close);
overlay?.addEventListener('click', close);
document.querySelectorAll('.side-menu a').forEach(a => a.addEventListener('click', close));

const DATA = window.FAMILY_OFFERS || { categories: [], whatsapp: '9647740317888' };
const wa = DATA.whatsapp || '9647740317888';
const pills = document.getElementById('categoryPills');
const allOffers = document.getElementById('allOffers');

function whatsAppLink(text){ return `https://wa.me/${wa}?text=${encodeURIComponent(text)}`; }
...
function renderOffers(){
  ...
  pills.innerHTML = DATA.categories.map(c => `<a href="#cat-${c.id}">${c.title}</a>`).join('');
  allOffers.innerHTML = DATA.categories.map(cat => `
